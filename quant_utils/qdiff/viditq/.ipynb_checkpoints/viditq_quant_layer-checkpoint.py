...
class ViDiTQuantizedLinear(QuantizedLinear):
    ...
    def update_quantized_weight_rotated_and_scaled(self):
        ...
        if self.q_cfg.quantized:
            self.weight.data = self.w_quantizer(self.fp_module.weight / self.channel_mask.reshape([1, C_in]).cpu()).to("cuda")
            ...
            self.weight.data = self.w_quantizer(torch.matmul(self.weight.data.double().to("cuda"), self.rotation_matrix.to("cuda")).float()).cpu()
